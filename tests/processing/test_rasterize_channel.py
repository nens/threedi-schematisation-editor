# TODO add integration test / smoke test
...
from threedi_schematisation_editor.processing.deps.rasterize_channel import (
    ...
    find_wedge_channels,
    highest_valid_index,
    highest_valid_index_single_offset,
    is_valid_offset,
...
WALL_DISPLACEMENT = (
    0.01  # tops or bottoms of vertical segments are moved by this amount
)
...
if not (geos_version[0] > 3 or (geos_version[0] == 3 and geos_version[1] >= 12)):
    raise Exception(
        f"Required GEOS version >= 3.12.0. Installed GEOS version: {geos_version}"
    )
...
def get_test_channel(nr: int = 0):
    ...
    if nr == 0:
        ...
        wkb_geometry = (
            b'\x01\x02\x00\x00\x00\x03\x00\x00\x00\xc6\xd2w\xb3Vo"@\x00\xcc)C\x13S\xa1?\x13\xc9nJ\x8c\x08'
        ...
        wkb_geometry = (
            b"\x01\x02\x00\x00\x00\x03\x00\x00\x00b\xd4\x90\x9d\xb6lI@\x00[\x95\x12UM\xc5?\xc7\xf5"
...
def get_test_cross_section_location(nr: int = 0):
    if nr == 0:
        ...
        y, z = parse_cross_section_table(
            table="0, 5\n5, 0\n7, 3", cross_section_shape=SupportedShape.YZ.value
        )
...
def cross_section_locations_are_equal(
    reference: CrossSectionLocation, compare: CrossSectionLocation
) -> bool:
...
def test_parse_cross_section_table():
    ...
    assert np.all(
        y == np.array([0, 1, 1 + WALL_DISPLACEMENT, 3, 3 + WALL_DISPLACEMENT, 4])
    )
...
def test_is_valid_offset():
    # VALID
    assert (
        is_valid_offset(LineString([[0, 0], [0, 1], [0, 2], [0, 3], [1, 1]]), -0.5)
        is True
    )
    ...
    assert (
        is_valid_offset(
            LineString([[0, 0], [0, 1], [0, 2], [0, 3], [1, 1], [1, 0]]), -0.5
        )
        is False
    )
...
def test_highest_valid_index(plot: bool = False):
    ...
    if plot:
        ...
        plt.plot(
            point_x, point_y, "ro", label=f"HVI for small offset: {hvi_small_offset}"
        )
        hvi_large_offset = highest_valid_index(line, [offset_large])
        point_x, point_y = line.coords[hvi_large_offset]
        plt.plot(
            point_x, point_y, "ko", label=f"HVI for large offset: {hvi_large_offset}"
        )
        plt.xlabel("X-axis")
        plt.ylabel("Y-axis")
        plt.axis("equal")  # Set equal scaling
    ...
    assert highest_valid_index(
        line, [offset_small, offset_large]
    ) == highest_valid_index(line, [offset_large])
...
def test_channel_properties():
    ...
    assert np.all(
        ...
        == np.array([0, 0.5, 1]) * channel.geometry.length
    )
    ...
    if plot:
        ...
        def plot_channel_and_sections(
            ...
            if channel.geometry is not None:
                ...
                ax.plot(
                    x, y, color=channel_color, linewidth=2, label="Channel centerline"
                )
...
def test_channel_split(plot: bool = False):
    ...
    if plot:
        plot_it(
            channel, split_idx, title="Split point is before the cross-section location"
        )
    # - output types
    ...
    assert (
        len(split_channel[1].geometry.coords)
        == len(channel.geometry.coords) - split_idx
    )

    # - cross-section locations
    assert len(split_channel[0].cross_section_locations) == 1
    assert len(split_channel[1].cross_section_locations) == 1
    assert cross_section_locations_are_equal(
        split_channel[0].cross_section_locations[0], xsec
    )
    assert cross_section_locations_are_equal(
        split_channel[1].cross_section_locations[0], xsec
    )

    # - cross-section location positions
    assert np.array_equal(
        split_channel[0].cross_section_location_positions,
        channel.cross_section_location_positions,
    )
    assert np.array_equal(
    ...
    assert split_channel[0].connection_node_id_end == (
        channel.connection_node_id_start[0],
        channel.connection_node_id_start[1] + 1,
    )
    assert (
        split_channel[1].connection_node_id_start
        == split_channel[0].connection_node_id_end
    )
    ...
    if plot:
        plot_it(
            channel, split_idx, title="Split point is after the cross-section location"
        )
    ...
    if plot:
        plot_it(
            channel,
            split_idx,
            title="Split point is in between two cross-section locations",
        )
    ...
    assert cross_section_locations_are_equal(
        split_channel[0].cross_section_locations[0], channel.cross_section_locations[0]
    )
    assert cross_section_locations_are_equal(
        split_channel[0].cross_section_locations[1], channel.cross_section_locations[1]
    )
    assert cross_section_locations_are_equal(
        split_channel[1].cross_section_locations[0], channel.cross_section_locations[0]
    )
    assert cross_section_locations_are_equal(
        split_channel[1].cross_section_locations[1], channel.cross_section_locations[1]
    )
    ...
    if plot:
        plot_it(
            channel,
            split_idx,
            title="Two cross-section locations after the split point",
        )
    ...
    assert np.allclose(
        split_channel[0].cross_section_location_positions,
        channel.cross_section_location_positions[0],
    )
    assert np.allclose(
        split_channel[1].cross_section_location_positions,
        channel.cross_section_location_positions - split_channel[0].geometry.length,
    ...
    if plot:
        plot_it(
            channel,
            split_idx,
            title="Two cross-section locations before the split point",
        )
    ...
    assert cross_section_locations_are_equal(
        split_channel[1].cross_section_locations[0], channel.cross_section_locations[1]
    )
...
def test_cross_section_starting_at_0_0():
    ...
    wkt_geometry = (
        "LineString (94066.74041438 441349.75156281, 94060.74041445 441355.7515628, 94064.24041445 "
...
def test_parallel_offset_heights_at_vertices():
    ...
    assert np.allclose(
        po.heights_at_vertices,
        np.array(
            [3.0, 3.0, 3.0, 3.00655041, 3.26610821, 4.72680547, 4.73884013, 5.0, 5.0]
        ),
    )
...
def test_fill_wedge(plot: bool = False):
    if plot:
        ...
        def plot_channel_triangles(
            channels, ax=None, show=True, title="", highlight: Channel = None
        ):
            """
            ...
            for channel in channels:
                ...
                plt.arrow(
                    x[-2],
                    y[-2],  # start from the second-to-last point
                    x[-1] - x[-2],
                    y[-1] - y[-2],  # direction vector
                    shape="full",
                    lw=0,
    ...
    cross_section_loc_channel_3.geometry = Point(
        channel_3.geometry.coords[1]
...
def test_triangle(plot: bool = False):
    ...
    if plot:
        plot_triangle_with_lines(
            tri,
            line_1,
            LineString([point_line, point_line]),
            "Case 0: one line is a Point",
        )
    ...
    if plot:
        plot_triangle_with_lines(
            tri, line_1, line_2, "Case 1: triangle side crosses one line"
        )
    ...
    if plot:
        plot_triangle_with_lines(
            ...
            line_1,
            line_2,
            "Case 2.1: one side shared, opposite point touches other line",
        )
...
def test_triangulate_between(plot: bool = False):
    if plot:
        ...
        def plot_triangles_and_points(
            ...
            for tri in triangles:
                ...
                ax.fill(
                    x, y, facecolor=triangle_face, edgecolor=triangle_edge, alpha=0.8
                )

            # Plot first set of points
            if points_a:
                ax.scatter(
                    [p.geom.x for p in points_a],
                    [p.geom.y for p in points_a],
                    color=color_a,
                    s=40,
                    label="Points A",
                )
            ...
            if points_b:
                ax.scatter(
                    [p.geom.x for p in points_b],
                    [p.geom.y for p in points_b],
                    color=color_b,
                    s=40,
                    marker="x",
                    label="Points B",
    ...
    if plot:
        plot_triangles_and_points(
            triangles, points_1, points_2, title="Base case: two parallel lines"
        )
    ...
    points_2 = [IndexedPoint(50, 0, 10, index=5), IndexedPoint(45, 2.5, 15, index=9)]
    triangles = [
        triangle
        for triangle in triangulate_between(
            left_side_points=points_1,
    ...
    if plot:
        plot_triangles_and_points(
            triangles, points_1, points_2, title="Side 1 has only 1 point (corner case)"
        )
    assert len(triangles) == 1
    assert (
        triangles[0].geometry.wkt
        == "POLYGON Z ((50 5 15, 50 0 10, 45 2.5 15, 50 5 15))"
    )
