...
class JoinFieldsRow(QDialog):
    ...
    def value(self):
        if self.input_cbo.currentText() == "" and self.input_expr.isValidExpression():
            return {"method": "expression", "expression": self.input_expr.expression()}
        else:
            return {
                "method": "source_attribute",
                "source_attribute": self.input_cbo.currentText(),
            }

    @value.setter
    def value(self, value):
        if value.get("method") == "expression":
            self.expr_radio.setChecked(True)
            self.input_expr.setExpression(value.get("expression"))
        elif value.get("source_attribute"):
            self.attr_radio.setChecked(True)
            self.input_cbo.setCurrentText(value.get("source_attribute"))
